...
def find_aspects(sentences, city, n_top_words=15):
    ...
    for i in important_words:
        if i not in [city, city.lower(),'okay','ok','thing','things','time','times','greasy','awful','review','oh','sorry','restaurant'\
        'restaurants','reviews','experiece','alice','unit','man','dry','hour','lo','food','drink','minute','minutes','cute','luck','excellent','friend'\
        'elephant','decent','thai','hut','friends','restaurant','restaurants','decent','soooo','bears','people','friend','mom','children'] and TextBlob(i).tags[0][1] in ['NN', 'NNS']:
            for j in noun_list:
                if i in j.split(' '):
...
def find_summary(sentences, aspects_dict, topic, city):
    ...
    for sentence in sentences:
        for aspect in aspects_dict[topic]:
            if sentence.find(aspect)!= -1 and TextBlob(sentence).sentiment[0]<-0.2:
...
if __name__ == "__main__":
    ...
    sens= make_sentences(bad_review, business_id)
